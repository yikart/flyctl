...
build: generate
	@echo Running Build
	CGO_ENABLED=0 go build -o bin/flyctl -ldflags="-X 'github.com/yikart/flyctl/internal/buildinfo.buildDate=$(NOW_RFC3339)' -X 'github.com/yikart/flyctl/internal/buildinfo.branchName=$(GIT_BRANCH)'" .

test: FORCE
	go test ./... -ldflags="-X 'github.com/yikart/flyctl/internal/buildinfo.buildDate=$(NOW_RFC3339)'" --run=$(T)
...
install-fake-production:
	go install -tags production -ldflags="-X 'github.com/yikart/flyctl/internal/buildinfo.buildDate=2000-01-01T00:00:00Z' -X 'github.com/yikart/flyctl/internal/buildinfo.buildVersion=9.9.9'" .
